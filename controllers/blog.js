...
exports.createBlog = (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    if (!body || body.length < 200) {
      return res.status(400).json({ error: 'body is too short' });
    }

    if (!categories || categories.length === 0) {
      return res
        .status(400)
        .json({ error: 'At least one category should be select' });
    }

    if (!tags || tags.length === 0) {
      return res
        .status(400)
        .json({ error: 'At least one tag should be select' });
    }
...
exports.listAllBlogsCategoriesTags = (req, res) => {
  ...
  Blog.find({})
    ...
    .exec((err, data) => {
      ...
      blogs = data;
      // get all categories
      Category.find({}).exec((err, categoryResults) => {
        if (err) {
          return res.status(400).json({ error: errorHandler(err) });
        }
...
exports.updateBlog = (req, res) => {
  ...
  Blog.findOne({ slug }).exec((err, oldBlog) => {
    ...
    form.parse(req, (err, fields, files) => {
      ...
      if (body) {
        oldBlog.body = smartTrim(body, 320, ' ', ' ...');
...
exports.listRelatedBlogs = (req, res) => {
  ...
  Blog.find({ _id: { $ne: _id }, categories: { $in: categories } })
    ...
    .exec((err, blogs) => {
      if (err) {
        return res.status(400).json({ error: 'Related Blogs not Found' });
      }
      res.json(blogs);
      console.log(blogs);
    });
